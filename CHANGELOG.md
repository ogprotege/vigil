...
- The Home card's spoken VoiceOver summary now follows the same freshness honesty as the visible line: a degraded account says "Data from <time>" instead of "Last checked <time>", because the card's accessibility label replaces its children and had kept the misleading check-time claim for VoiceOver users. A UI test reads the drifted demo card's spoken label to pin the surface itself, not just the helper. The history-recovery removal dialog — same dialog wiring class as the fixed Remove account bug, previously unreachable in automation — is now walked end to end in UI tests through a demo-only damaged-history mode that never touches shared stores.
