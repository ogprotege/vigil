...
An on-device AI usage monitor. It polls Claude and ChatGPT/Codex subscription windows, plus 12 opt-in providers (OpenRouter, DeepSeek, Moonshot/Kimi and its China variant, MiniMax and its China variant, OpenAI, GitHub Copilot, xAI, Z.ai, Cursor, and Kimi K3 coding plan), directly from the user's phone or Mac. MiniMax global/China, Z.ai, Cursor, and Kimi K3 use experimental community-researched contracts. Moonshot global/China and xAI are vendor-documented. There is **no Vigil server**. Credentials are provisioned on the phone and live in the device Keychain (ADR-0001): Claude via on-device "Sign in with Claude" OAuth, Codex via OpenAI's device-code sign-in, and each key-based provider by pasting its credential. The `vigil1` QR handoff via `npx vigil-link` is an **optional** way to reuse a Claude Code / Codex sign-in that already exists on a computer.
