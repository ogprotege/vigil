...
| Grok Build (`grok`) | Guided xAI device authorization for setup and re-linking; manual session-token paste remains for recovery. | Shared weekly credit utilization and reset from the current `format=credits` response; compatible older responses may supply exact monthly used/limit values; optional on-demand used and cap when the cap is positive. | Live-sanitized current weekly response, retired zero-monthly-limit response, and older monthly response. The cli-chat-proxy billing endpoint is undocumented. | Experimental |
