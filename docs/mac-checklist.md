...
> **Status (2026-07-22):** steps 1–4 below are ✅ done (foundation validated
> live, `vigil-link@0.1.1` published). M4–M7 are built and M8 shipped —
> build **0.13.0 (13)** was uploaded successfully to TestFlight and is
> processing for the Internal group (see
> [release.md](release.md)). Shipped since 0.9.0 (1): **0.10.0** (13-provider
> expansion), **0.11.0** (UI redesign), and **0.12.0** (phone-native setup +
> per-model limits), followed by **0.13.0** (provider-contract correction and
> fail-closed drift detection). **The remaining work is steps 5–17: the
> on-device walk**,
...
8. **Add account → Sign in with Claude** (primary): approve in the browser, paste the code back → on-device token mint → live verify succeeds → dashboard shows real percentages. *(Sign-in completed on build 10; repeat on build 13 to verify corrected session, weekly, and model presentation.)*
8a. **Add account → Sign in with Codex** (primary): approve the device code in the browser → Vigil polls and mints its own token pair → poll completes and the account goes live. *(Sign-in completed on build 10; repeat on build 13 to verify corrected plan and model lanes.)*
